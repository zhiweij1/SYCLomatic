...
void foo() {
  ...
  //      CHECK: status = DPCT_CHECK_ERROR(dpct::blas::trmm(handle->get_queue(), side, uplo, transa, diag, m, n, alpha_s, A_s, lda, B_s, ldb, C_s, ldc));
  // CHECK-NEXT: status = DPCT_CHECK_ERROR(dpct::blas::trmm(handle->get_queue(), side, uplo, transa, diag, m, n, alpha_d, A_d, lda, B_d, ldb, C_d, ldc));
  // CHECK-NEXT: status = DPCT_CHECK_ERROR(dpct::blas::trmm(handle->get_queue(), side, uplo, transa, diag, m, n, alpha_c, A_c, lda, B_c, ldb, C_c, ldc));
  // CHECK-NEXT: status = DPCT_CHECK_ERROR(dpct::blas::trmm(handle->get_queue(), side, uplo, transa, diag, m, n, alpha_z, A_z, lda, B_z, ldb, C_z, ldc));
  ...
  //      CHECK: status = DPCT_CHECK_ERROR(dpct::blas::syrk(handle->get_queue(), uplo, transa, n, k, alpha_s, A_s, lda, B_s, ldb, beta_s, C_s, ldc));
  // CHECK-NEXT: status = DPCT_CHECK_ERROR(dpct::blas::syrk(handle->get_queue(), uplo, transa, n, k, alpha_d, A_d, lda, B_d, ldb, beta_d, C_d, ldc));
  // CHECK-NEXT: status = DPCT_CHECK_ERROR(dpct::blas::syrk(handle->get_queue(), uplo, transa, n, k, alpha_c, A_c, lda, B_c, ldb, beta_c, C_c, ldc));
  // CHECK-NEXT: status = DPCT_CHECK_ERROR(dpct::blas::syrk(handle->get_queue(), uplo, transa, n, k, alpha_z, A_z, lda, B_z, ldb, beta_z, C_z, ldc));
  ...
  //      CHECK: status = DPCT_CHECK_ERROR(dpct::blas::herk(handle->get_queue(), uplo, transa, n, k, alpha_c, A_c, lda, B_c, ldb, beta_s, C_c, ldc));
  // CHECK-NEXT: status = DPCT_CHECK_ERROR(dpct::blas::herk(handle->get_queue(), uplo, transa, n, k, alpha_z, A_z, lda, B_z, ldb, beta_d, C_z, ldc));
